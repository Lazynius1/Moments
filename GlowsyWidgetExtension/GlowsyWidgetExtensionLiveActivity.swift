        ...
        ActivityConfiguration(for: StoryUploadActivityAttributes.self) { context in
            ...
            VStack(alignment: .leading, spacing: 8) {
                HStack {
                    ...
                    Text(NSLocalizedString("liveActivity.uploadingStory", comment: "Uploading story title"))
                        .font(.headline)
                    
                    Spacer()
                    
                    if context.state.status == "completed" {
                        Text(NSLocalizedString("liveActivity.completed", comment: "Completed status"))
                ...
                DynamicIslandExpandedRegion(.leading) {
                    HStack(spacing: 8) {
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            ...
                            Text(context.state.status == "uploading" ? NSLocalizedString("liveActivity.uploading", comment: "Uploading status") :
                                 context.state.status == "processing" ? NSLocalizedString("liveActivity.processing", comment: "Processing status") :
                                 context.state.status == "completed" ? NSLocalizedString("liveActivity.completed", comment: "Completed status") : NSLocalizedString("liveActivity.error", comment: "Error status"))
                                .font(.caption)
