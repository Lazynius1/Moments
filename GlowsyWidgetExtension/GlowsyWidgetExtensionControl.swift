...
extension GlowsyWidgetExtensionControl {
    ...
    struct Provider: AppIntentControlValueProvider {
        func previewValue(configuration: TimerConfiguration) -> Value {
            GlowsyWidgetExtensionControl.Value(isRunning: false, name: configuration.timerName)
        }

        func currentValue(configuration: TimerConfiguration) async throws -> Value {
            let isRunning = true // Check if the timer is running
            return GlowsyWidgetExtensionControl.Value(isRunning: isRunning, name: configuration.timerName)
        }
    }
}
