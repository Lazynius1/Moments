...
struct GroupedVisitRow: View {
    ...
    @State var showExpandedVisits = false
...
class VisitsViewModel: ObservableObject {
    ...
    func analyzeStalkers(allVisits: [Visit], userDict: [String: AppUser]) {
        let now = Date()
        ...
        for (visitorId, visits) in visitorGroups {
            ...
            guard frequencyType != VisitorFrequencyType.normal else { continue }
        ...
        DispatchQueue.main.async {
            ...
            if let superStalker = analyses.first(where: { $0.frequencyType == VisitorFrequencyType.superStalker }) {
