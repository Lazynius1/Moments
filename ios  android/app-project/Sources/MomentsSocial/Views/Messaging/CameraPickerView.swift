...
struct EnhancedCameraPickerView: View {
    ...
    @State var isEphemeralMode = false
    @State var captureMode: CaptureMode = .photo
    @State var cameraPosition: CameraPosition = .back
    @State var flashMode: FlashMode = .off
    @State var selectedItems: [PhotosPickerItem] = [] // ✅ Para PhotosPicker
    @State var showGridLines = false // ✅ Para grid lines
    @State var showPhotoPicker = false // ✅ Control del PhotosPicker
    ...
    @State var cameraViewController: CameraViewController?
    ...
    private var topControlsBar: some View {
        HStack {
            ...
            Button(action: { toggleEphemeralMode() }) {
                ...
                .background(
                    Capsule()
                        .fill(isEphemeralMode ? Color.yellow : Color.black.opacity(0.6))
                        .overlay(
                            Capsule()
                                .stroke(isEphemeralMode ? Color.yellow.opacity(0.8) : Color.white.opacity(0.2), lineWidth: 1)
                        )
                )
            }
        }
                ...
                Button(action: {
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 12)
                            .fill(captureMode == mode ? Color.yellow.opacity(0.2) : Color.clear)
                            .overlay(
                                RoundedRectangle(cornerRadius: 12)
                                    .stroke(captureMode == mode ? Color.yellow : Color.clear, lineWidth: 2)
                            )
                    )
                }
                ...
                Circle()
                    ...
                    .overlay(
                        Image(systemName: showGridLines ? "grid.circle.fill" : "grid.circle")
                            .font(.system(size: 20))
                            .foregroundColor(showGridLines ? .yellow : .white)
                    )
            }
        }
        .padding(.bottom, 50)
    ...
    var body: some View {
        Button(action: handleCapture) {
            ZStack {
                ...
                Circle()
                    .stroke(
                        isEphemeralMode ? Color.yellow : Color.white,
                ...
                Group {
                    if isRecording {
                        ...
                        Circle()
                            .fill(isEphemeralMode ? Color.yellow : Color.white)
                            .frame(width: 70, height: 70)
                            .overlay(
                                Image(systemName: captureMode.icon)
                                    .font(.system(size: 24, weight: .medium))
                                    .foregroundColor(isEphemeralMode ? .black : .black)
...
// Android: Camera view controller will be handled natively
