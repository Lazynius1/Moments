...
struct MessagingView: View {
    ...
    @State var isShowingNewConversation = false
    @State var selectedConversation: Conversation? // ✅ Solo para navigationDestination
    @Binding var targetConversationId: String?
    @State var offsetValues: [String: CGFloat] = [:]
    ...
    @State var userStories: [String: (hasStory: Bool, hasUnseenStory: Bool)] = [:]
    // ✅ SOLICITUDES: Estado para mostrar solicitudes
    @State var showingMessageRequests = false
    @State var pendingRequestCount = 0
    
    // ✅ NUEVO: Estado para el selector de estados online
    @StateObject var onlineStatusService = OnlineStatusService()
    @State var showingStatusSelector = false
...
struct GlassmorphicConversationRow: View {
    ...
    @State var storiesUserId: String = ""
            ...
            .getDocuments { snapshot, error in
                ...
                group.notify(queue: DispatchQueue.main) {
                    if !visibleStories.isEmpty {
                        ...
                        viewGroup.notify(queue: DispatchQueue.main) {
...
struct GlassmorphicNewConversationView: View {
    ...
    @State var showingMessageComposer = false
