...
struct LocationMapView: View {
    ...
    @State var annotations: [MapsLocationAnnotation] = []
    @State var isLoading = true
    @State var errorMessage: String?
    @State var locationMoments: [Moment] = []  // ✅ USAR MOMENT EN VEZ DE LocationMoment
    @State var isLoadingMoments = false
    @State var showingGallery = false
    @State var selectedMoment: Moment?  // ✅ USAR MOMENT
    @State var showingProfile = false
    @State var showingDetail = false
    @State var selectedMomentIndex = 0
    
    @State var locationPermissionGranted = false
    @StateObject var locationManager = LocationUtilities.shared
    
    @StateObject var weatherService = WeatherService.shared
    @State var currentWeather: WeatherData?
    @State var weatherEffectsEnabled = true
    @State var showingBottomSheet = false
    @State var bottomSheetOffset: CGFloat = 300
    @State var isDragging = false
    
    // ✅ NUEVO: Estado para manejar mejor la carga inicial
    @State var hasInitializedMap = false
...
extension LocationMapView {
    ...
    // ✅ EFECTOS DE PARTÍCULAS (LLUVIA/NIEVE)
    // Android: Particle effects will be handled natively
...
struct ModernLocationGalleryView: View {
    ...
    let colorScheme: ColorScheme
    @Binding var isPresented: Bool
    @State var selectedMoment: Moment?  // ✅ CAMBIO AQUÍ
    @State var showingDetail = false
                    ...
                    LazyVGrid(columns: columns, spacing: 2) {
                        ForEach(moments) { moment in
                            Button(action: {
                                ...
                                // ✅ USAR moment.imagePath EN VEZ DE moment.imageUrl
                                AsyncImage(URL(string: moment.imagePath ?? ""))
                                    .placeholder {
                                        Rectangle()
                                            .fill(.ultraThinMaterial)
                                            .overlay(
...
struct LocationBottomSheet: View {
    ...
    @State var isDragging = false
    @State var viewMode: ViewMode = .gallery
...
extension View {
    func cornerRadius(_ radius: CGFloat, corners: UIRectCorner) -> some View {
        let cornerBits: UInt = corners.rawValue
        return clipShape(RoundedCorner(radius: radius, corners: cornerBits))
    }
}

struct RoundedCorner: Shape {
    var radius: CGFloat = .infinity
    var corners: UInt = 15 // All corners by default (bits: topLeft | topRight | bottomLeft | bottomRight)

    func path(in rect: CGRect) -> Path {
        // Android: Simplified rounded corners
