...
struct FeedView: View {
    ...
    @StateObject var storyUploadService = BackgroundStoryUploadService.shared
    @StateObject var adManager = NativeAdManager()
    @StateObject var notificationSummaryService = NotificationSummaryService.shared
    @ObservedObject var badgeService = NotificationBadgeService.shared // ✅ NUEVO
    @StateObject var navigationService = NotificationNavigationService.shared
    @StateObject var networkMonitor = NetworkMonitor.shared // ✅ NUEVO: NetworkMonitor
    let privacyService = PrivacyService()
    @State var showNotifications = false
    @State var showMessages = false
    @State var showStories = false
    @State var showingComments = false
    @State var selectedMoment: Moment?
    @Binding var showCreatorView: Bool
    @State var currentTime = Date()
    @Environment(\.colorScheme) var colorScheme
    @State var storyUsers: [(userId: String, hasStory: Bool, hasUnseenStory: Bool)] = []
    @State var isLoadingStories = true
    @State var selectedFeedType: FeedType = UserDefaults.standard.selectedFeedType
    @State var showingLocationMap = false
    @State var selectedLocationName: String = ""
    @State var selectedLocationCoordinate: CLLocationCoordinate2D?
    @State var showUserProfile = false
    @State var selectedUserId: String = ""
    // 🔗 STORY CHAINS: Variables para navegación
    @State var showStoryChain = false
    @State var selectedChainId: String = ""
    @State var selectedChainTitle: String = ""
    // ✅ NUEVO: Cache básico para optimización
    @State var cachedStories: [String: Bool] = [:]
    @State var cachedUnseenStories: [String: Bool] = [:]
    @State var cachedStoriesTimestamp: Date = Date()
    @State var hasLoadedInitialData = false
    // ✅ NUEVO: Mapa global
    @State var hasUnreadMessages: Bool = false
    @State var showSpecificUserStories = false
    @State var selectedStoryUserId: String = ""
    @State var selectedHashtag: String = ""
    @State var showExploreWithHashtag = false
    @State var showGlobalContextMenu = false
    @State var selectedMomentForMenu: Moment?
    @State var showShareSheet = false
    @State var showEditSheet = false
    @State var showDeleteAlert = false
    @State var showReportSheet = false
    @State var editedContent = ""
    // ✨ NUEVO: Variables para ReactionsListSheet

    @State var isDeleting = false
    @State var targetConversationId: String? = nil
    @State var targetMomentId: String? = nil
    @State var showMomentDetail = false
    @State var targetMomentUserId: String? = nil
    ...
    struct YourStoryCircleWithProgress: View {
        ...
        var body: some View {
            ...
            .scaleEffect((storyUploadService.uploadingStory?.status == UploadStatus.failed) == true ? 0.95 : 1.0)
        ...
        var baseStoryRingGradient: LinearGradient {
            if hasStory {
    ...
    struct UploadProgressRow: View {
        ...
        @State var checkScale: CGFloat = 1.0
        
        var body: some View {
            HStack(spacing: 12) {
                // Thumbnail pequeño
                if uploadingMoment.thumbnailImage != nil {
...
struct ModernStoryButton: View {
    let colorScheme: ColorScheme
    let action: () -> Void
    @StateObject var uploadProgressManager = StoryUploadProgressManager.shared
...
struct ModernPostCardView: View {
    ...
    @State var detectedAspectRatio: CGFloat = 1.0
    @State var isFollowing: Bool = false
    @State var isSaved: Bool = false
    @State var isFollowLoading: Bool = false
    @State var isSaveLoading: Bool = false
    @State var commentCount: Int = 0
    @State var hasLoadedInitialData: Bool = false
    
    // ✅ ACTUALIZADO: AspectRatioType mejorado con soporte para reels
    @State var aspectRatioType: AspectRatioType = .square
    @State var cachedCardHeight: CGFloat?
    @State var lastCalculatedSize: CGSize = .zero
    ...
    @State var isLoadingStory: Bool = false
    @State var showStories = false
    @State var showSpecificUserStories = false
    ...
    private func detectAspectRatio() {
        ...
        if firstItem.type == .image {
            ...
            AsyncImage(URL(string: firstItem.url))
                .onSuccess { result in
...
public struct EnhancedCarouselView: View {
...
struct MediaItemView: View {
    ...
    @State var showReelsViewer = false
    ...
    var body: some View {
        Group {
            if item.type == .image {
                ...
                AsyncImage(URL(string: item.url))
                    .placeholder {
                        RoundedRectangle(cornerRadius: 20)
                            .fill(.ultraThinMaterial)
...
struct ExpandableContentView: View {
    ...
    @State var needsExpansion: Bool = false
