...
struct UserProfileView: View {
    ...
    @State var showingUserList: UserProfileView.UserListType?
    private let userId: String
    @StateObject var messagingViewModel = MessagingViewModel()
    @StateObject var messageRequestService = MessageRequestService()
    @State var navigateToChat: Bool = false
    @State var targetConversation: Conversation?
    @State var showingMessageRequestAlert = false
    @State var messageRequestText = ""
    @State var messageRequestError: String?
    @State var showingSuccessMessage = false
    @State var selectedMoment: Moment?

    @StateObject var storyViewModel = StoryViewModel()
    @State var showStoryViewer: Bool = false
    @State var selectedStoryIndex: Int = 0
...
struct UserModernInterestsView: View {
    ...
    @State var currentUserInterests: [String] = []
    @Environment(\.colorScheme) var colorScheme
...
struct UserModernMomentThumbnail: View {
    ...
    @State var isLoadingVideoThumbnail = false
...
class UserProfileViewModel: ObservableObject, UserListViewModel {
    ...
    private func fetchUsersInBatches(userIds: [String], completion: @escaping ([AppUser]) -> Void) {
        ...
        batchGroup.notify(queue: DispatchQueue.main) {
            completion(allUsers)
        }
    }
