...
struct SocialVideoEditorView: View {
    ...
    @State var trimStartTime: Double = 0
    @State var trimEndTime: Double = 0
    @State var selectedClipIndex = 0
    @State var playbackSpeed: PlaybackSpeed = .normal
    @State var selectedFormat: VideoFormat = .reels
    @State var volume: Float = 1.0
    
    // Estados de UI
    @State var showingSpeedPicker = false
    @State var showingFormatPicker = false
    @State var isDraggingTrimHandle = false
    @State var trimHandleType: TrimHandleType = .start
    @State var showingVolumeSlider = false
    @State var draggingHandle: TrimHandleType? = nil
    @State var lastDragLocation: CGPoint = .zero
    
    // Estados para compresión y procesamiento
    @State var isProcessing = false
    @State var processingMessage = "Procesando..."
    @State var processingProgress: Double = 0
    @State var showingError = false
    @State var errorMessage = ""
    
    // Estados para thumbnails del timeline
    @State var timelineThumbnails: [UIImage] = []
    @State var isGeneratingThumbnails = false
...
// Android: Video player will be handled natively
struct VideoPlayerWrapper: View {
    ...
    var body: some View {
        Color.black
            .onAppear {
                // Android: Video playback will be handled natively
            }
    }
}
