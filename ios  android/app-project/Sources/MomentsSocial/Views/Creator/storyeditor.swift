...
struct StoryEditingView: View {
    ...
    @State var storyText = ""
    @State var textPosition: CGPoint = CGPoint(x: UIScreen.main.bounds.width / 2, y: 100)
    @State var selectedStickers: [StickerItem] = []
    @State var showingTextEditor = false
    @State var showingStickerPicker = false
    @State var showingDrawing = false
    @State var isPublishing = false
    @State var storyAudience: CaptionAndDetailsView.AudienceSetting = .everyone
    @State var isLoadingUserSettings = true // NUEVO
    @Environment(\.colorScheme) var colorScheme
    @State var showingAudienceSelector = false
    @State var selectedTextStyle: TextStyle = .modern
    @State var drawingImage: UIImage?
    @State var editableImageViewRef: EditableImageView?
    
    // ✅ Variables para transformaciones de imagen
    @State var imageScale: CGFloat = 1.0
    @State var imageOffset: CGSize = .zero
    @State var imageRotation: Angle = .zero
    ...
    @State var isCreatingChain = false
    @State var chainTitle = ""
    @State var chainId: String? = nil
    @State var chainPosition: Int? = nil
    @State var isContinuingChain = false
    @State var originalChainTitle = ""
    @FocusState var isChainTitleFocused: Bool
    
    // 🔗 NUEVAS VARIABLES para configuración de cadenas
    @State var allowOthersToContinue = true
    @State var continuationAudience: ChainContinuationSetting = .everyone
    @State var showingChainConfiguration = false
...
struct StickerItem: Identifiable {
    ...
    let image: Any  // Para fallback y rendering final (Android: Any)
    ...
    init(image: Any, position: CGPoint, type: StickerType, interactionData: StickerInteractionData?) {
    ...
    init(image: Any, gifURL: URL, position: CGPoint, type: StickerType, interactionData: StickerInteractionData?) {
        self.id = "\(type.rawValue)_\(Int(position.x))_\(Int(position.y))"
        self.image = image
        self.position = position
        self.type = type
...
struct CameraPreviewRepresentable: UIViewRepresentable {
    ...
    let onImageCaptured: (Any) -> Void  // Android: Any instead of UIImage
