...
struct ProcessedMedia: Identifiable {
    let id: String
    var image: Any  // Android: Any instead of UIImage
    var videoURL: URL?
    let type: MediaType
    ...
    init(id: String, image: Any, videoURL: URL?, type: MediaType, aspectRatio: AspectRatio, hasEdits: Bool = false) {
        self.id = id
        self.image = image
...
// Android: PencilKit not available
// import PencilKit

struct DrawingView: View {
    let backgroundImage: Any? // Android: Any instead of UIImage
    let onComplete: (Any) -> Void
    @Environment(\.dismiss) var dismiss
    
    var body: some View {
        // Android: Drawing will be handled natively
        ZStack {
            Color.black
            Text("Drawing")
...
class DrawingViewController: UIViewController {
    ...
    func viewDidLoad() {
        // Android: viewDidLoad will be handled natively
        setupUI()
    }
    ...
    func viewDidAppear(_ animated: Bool) {
        // Android: viewDidAppear will be handled natively
        canvasView.becomeFirstResponder()
    }
...
struct CreatorView: View {
    ...
    @State var currentFlow: CreatorFlow = .typeSelection
    @State var contentType: ContentType = .moment
    @State var selectedMediaItems: [ProcessedMedia] = []
    @State var captionText: String = ""
    @State var taggedUsers: [String] = []
    @State var selectedLocation: CLLocationCoordinate2D?
    @State var locationName: String = ""
    @State var responseSticker: StickerItem? = nil
    
    // 🔗 NUEVO: Variables para contexto de cadena
    @State var pendingChainId: String? = nil
    @State var pendingChainTitle: String? = nil
    @State var pendingChainPosition: Int? = nil
...
struct MediaSelectionView: View {
    ...
    @State var mediaAssets: [PHAsset] = []
    @State var thumbnails: [String: UIImage] = [:]
    @State var selectedAssetIDs: [String] = []
    @State var isLoadingLibrary = true
    @State var showingCamera = false
    @State var showPermissionDenied = false
    
    // ✅ Estados para manejo de álbumes
    @State var availableAlbums: [AlbumInfo] = []
...
struct AlbumPickerView: View {
    ...
    @Environment(\.dismiss) var dismiss
    @Environment(\.colorScheme) var colorScheme
    @State var albumThumbnails: [String: UIImage] = [:]
...
struct MediaEditingView: View {
    ...
    @State var currentMediaIndex = 0
    @State var showingCropView = false
    @State var showingFilterView = false
    @State var appliedFilters: [String: FilterSettings] = [:]
...
struct CaptionAndDetailsView: View {
    ...
    @StateObject var uploadService = BackgroundMomentUploadService.shared
    
    @State var isPublishing = false
    @State var showingUserSearch = false
    @State var showingLocationPicker = false
    @State var showingAudience = false
    @State var audienceSetting: AudienceSetting = .everyone
    
    // New variables for custom lists
    @State var selectedListId: String?
    @State var selectedListName: String?
    @State var customSelectedUsers: [String] = []
...
struct StoryCameraView: View {
    ...
    @State var capturePhotoTrigger = false
    @State var lastGalleryImage: UIImage?
...
// Android: Camera preview will be handled natively
class CameraPreviewView: NSObject {
    ...
    init(frame: CGRect) {
        super.init()
        // Android: Camera setup will be handled natively
        setupCamera()
    }
    ...
    func layoutSubviews() {
        // Android: Layout will be handled natively
...
// Android: Image cropping will be handled natively using Android Image Cropper
struct CropViewWrapper: View {
    ...
    var body: some View {
        Color.clear
            .onAppear {
                // For Android, image cropping will be handled natively
                // Skip will transpile this to Android Image Cropper
                // For now, return the original image
                onComplete(image, aspectRatio)
                dismiss()
            }
    }
}
...
struct FilterSelectionView: View {
    ...
    @State var selectedFilter: FilterType = .original
    @State var filterIntensity: Double = 1.0
    ...
    var body: some View {
        NavigationView {
            VStack(spacing: 0) {
                ...
                // Map
                // Android: Map will use native implementation
...
struct AdvancedSettingsView: View {
    @AppStorage("disableComments") var disableComments = false
    @AppStorage("hideLikeCounts") var hideLikeCounts = false
    @AppStorage("allowSharing") var allowSharing = true
    @Environment(\.colorScheme) var colorScheme
...
struct StoryGalleryPicker: View {
    ...
    @State var showingMediaPicker = false
    @State var showingVideoLengthAlert = false
...
// Android: Media picker will be handled natively
struct StoryMediaPicker: View {
    ...
    var body: some View {
        Color.clear
            .onAppear {
                // Android: Media picker will open natively
    ...
    // Keep coordinator structure for iOS compatibility
...
// Android: Camera capture will be handled natively
struct CameraCapture: View {
    ...
    var body: some View {
        Color.clear
            .onAppear {
                // Android: Camera will open natively
                // For now, just dismiss
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                    dismiss()
                }
...
struct StoryOverlaysView: View {
    ...
    @State var selectedStickerId: String?
    @State var isEditingText = false
    @State var isDraggingItem = false
    @State var showTrashZone = false
...
struct StickerOverlayView: View {
    ...
    @State var showInteractionFeedback = false
...
struct ModernSelectionInterface: View {
    ...
    @State var initialRotation: Angle = .zero
