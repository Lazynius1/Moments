...
struct MomentDetailView: View {
    let moment: Moment
    @StateObject var viewModel: MomentDetailViewModel
    @Environment(\.dismiss) var dismiss
    @Environment(\.colorScheme) var colorScheme
    @State var showingCommentsSheet: Bool = false
    
    init(moment: Moment) {
        self.moment = moment
        self._viewModel = StateObject(wrappedValue: MomentDetailViewModel(momentId: moment.id ?? "", userId: moment.authorId))
    }
    ...
    @State var needsContentExpansion: Bool = false
    ...
    func detectAspectRatio() {
        ...
        if let savedAspectRatio = moment.aspectRatio {
            ...
            DispatchQueue.main.async {
                ...
                case ProcessedMedia.AspectRatio.landscape:
                    self.aspectRatioType = AspectRatioType.landscape
                case ProcessedMedia.AspectRatio.portrait:
                    self.aspectRatioType = AspectRatioType.portrait
                case ProcessedMedia.AspectRatio.square:
                    self.aspectRatioType = AspectRatioType.square
                case ProcessedMedia.AspectRatio.nineBySixteen:
                    self.aspectRatioType = AspectRatioType.reels // ✅ CORREGIDO: Usar reels para 9:16
                }
            }
            return
        }
        ...
        if firstItem.type == MediaItem.MediaType.image {
        ...
        } else if firstItem.type == MediaItem.MediaType.video {
...
struct VerticalReactionButton: View {
    let moment: Moment
    @State var currentReaction: ReactionType?
    @State var totalReactionCount: Int = 0
    @State var hasReacted: Bool = false
...
class MomentDetailViewModel: ObservableObject {
    ...
    func shareMoment() {
        // Android share will be handled natively using Android ShareSheet
        // Skip will transpile this to Android Intent
    }
