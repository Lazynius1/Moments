...
class StoryViewModel: ObservableObject {
    ...
    @Published var preloadedImages: [String: Any] = [:] // storyId: Image (Android: Any)
...
struct GlassmorphicStoryViewer: View {
    ...
    @State var imageTimer: Timer? = nil
    @State var isPaused: Bool = false
    @State var currentStoryId: String? = nil
    @State var messageText: String = ""
    @State var showReactions: Bool = false
    @State var showEphemeralPicker: Bool = false
    @State var selectedPhoto: PhotosPickerItem?
    @State var showQuickActions: Bool = false
    @State var showViewers: Bool = false
    @State var showShareSheet: Bool = false
    @State var dragOffset: CGFloat = 0
    @State var isDragging: Bool = false
    @State var showSuccessMessage: Bool = false
    @State var canContinueChain: Bool = false
    @State var successMessageText: String = ""
    @FocusState var isTextFieldFocused: Bool
    @Environment(\.dismiss) var dismiss
    @State var keyboardHeight: CGFloat = 0 // Track keyboard height
    @State var isKeyboardVisible: Bool = false // Track keyboard state
    @State var authorAllowsMessages: Bool = true
    @State var authorAllowsReactions: Bool = true
    @State var authorAllowsEphemeralPhotos: Bool = true
    @State var storyStickers: [StickerItem] = [] // Cache de stickers
    @State var showUserProfile = false
    @State var selectedUserId: String = ""
    // ✅ SOLO ZOOM - Estados para pinch to zoom
    @State var zoomScale: CGFloat = 1.0
    @State var lastZoomScale: CGFloat = 1.0
    // 🔗 STORY CHAINS - Variables para cadenas de historias
    @State var showChainView: Bool = false
    @State var selectedChainId: String = ""
    @State var selectedChainTitle: String = ""
    @State var chainStories: [Story] = [] // Todas las historias de la cadena
    @State var currentChainIndex: Int = 0 // Índice actual en la cadena
    @State var isLoadingChainStories: Bool = false
                ...
                HStack(spacing: 10) {
                    ZStack {
                        if let profileImagePath = story.profileImagePath {
                            AsyncImage(URL(string: profileImagePath))
                                .resizable()
                                .scaledToFill()
...
struct StoryReplyMessageBubble: View {
    ...
    @State var showEphemeralContent: Bool = false
...
struct EphemeralStoryReplyContent: View {
    ...
    @State var hasBeenViewed: Bool = false
...
struct InteractivePollOverlay: View {
    ...
    @State var hasVoted = false
    @State var voteCounts: [Int: Int] = [0: 0, 1: 0]
...
struct PollVoteView: View {
    ...
    @State var selectedOption: Int? = nil
    @State var hasVoted = false
    @State var voteCounts: [Int: Int] = [0: 0, 1: 0] // option index: count
...
struct StoryStickerView: View {
    ...
    @State var selectedPollOption: Int? = nil
    @State var hasVoted = false
    @State var voteCounts: [Int: Int] = [0: 0, 1: 0]
    @State var totalVotes = 0
...
struct InteractiveQuestionSticker: View {
    ...
    @State var showingResponseInput = false
    @State var showingResponsesView = false
    @State var responseText = ""
    @State var responseCount = 0
    @State var hasResponded = false
...
struct InteractiveHashtagSticker: View {
    ...
    let onPauseStory: () -> Void
    let onResumeStory: () -> Void
    @State var showingHashtagExplore = false
...
struct AnimatedWeatherSticker: View {
    ...
    @State var animationPhase: CGFloat = 0
