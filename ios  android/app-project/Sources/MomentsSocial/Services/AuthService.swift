...
class AuthService: ObservableObject {
    ...
    func register(username: String, email: String, password: String, interests: [String], privacyPolicyAccepted: Bool, profileImage: Any?, completion: @escaping (Result<Void, Error>) -> Void) {
        ...
        private func uploadProfileImageIfNeeded(image: Any?, userId: String, completion: @escaping (String?) -> Void) {
            guard let image = image else {
                completion(nil)
                return
            }
            
            // Android: Image data conversion will be handled natively
            // For now, we'll skip image upload and return nil
